...
- [invokes callback-form auth after the client factory returns](../src/handshake.test.ts#L49)
- [disconnect cancels a static connection while callback auth is unresolved](../src/handshake.test.ts#L63)
- [a reconnect replays the client-supplied auth on the fresh socket](../src/handshake.test.ts#L106)
- [a reconnect reads a replacement object from client.auth](../src/handshake.test.ts#L123)
- [a reconnect re-evaluates the current callback from client.auth](../src/handshake.test.ts#L139)
...
- [catch-all removal is a no-op for an unknown listener](../src/remove-listeners.test.ts#L107)
- [a listener removed during its own dispatch still runs for that dispatch](../src/remove-listeners.test.ts#L121)
- [off removes a once registration by its original listener](../src/remove-listeners.test.ts#L141)
- [off removes a once registration on the client side too](../src/remove-listeners.test.ts#L154)
- [server off removes the last match, leaving the earlier once registration](../src/remove-listeners.test.ts#L172)
- [server off removes the last match, leaving the earlier on registration](../src/remove-listeners.test.ts#L187)
- [client off removes the first match, leaving the later on registration](../src/remove-listeners.test.ts#L202)
- [client off removes the first match, leaving the later once registration](../src/remove-listeners.test.ts#L217)
- [client off removes only the named registration](../src/remove-listeners.test.ts#L235)
- [client removeAllListeners(event) clears that event only](../src/remove-listeners.test.ts#L252)
- [server off(event) without a listener throws](../src/remove-listeners.test.ts#L273)
- [client off(event) without a listener clears that event](../src/remove-listeners.test.ts#L279)
- [removeAllListeners() does not stop room cleanup](../src/remove-listeners.test.ts#L291)
- [removeAllListeners() leaves catch-all listeners in place](../src/remove-listeners.test.ts#L301)
...
- [close rejects a connection whose callback auth resolves after shutdown](../src/server-close.test.ts#L32)
- [close rejects a connection whose namespace middleware resolves after shutdown](../src/server-close.test.ts#L58)
...
- [Server listener methods delegate state and runtime identity to the root Namespace](../src/inherited-emitter.test.ts#L8)
- [Namespace prepend methods order connection listeners and expose once wrappers](../src/inherited-emitter.test.ts#L33)
- [ParentNamespace snapshots inherited listener ordering when each child is created](../src/inherited-emitter.test.ts#L63)
- [server Socket inherited methods preserve Node ordering and raw listener identity](../src/inherited-emitter.test.ts#L101)
- [named listener callbacks receive their Namespace or Socket receiver](../src/inherited-emitter.test.ts#L132)
- [Node emitter aliases remove the last matching registration](../src/inherited-emitter.test.ts#L202)
- [once wrapper identity properties remain specific to each emitter side](../src/inherited-emitter.test.ts#L218)
- [max-listener state is receiver-local and Server delegates it to root](../src/inherited-emitter.test.ts#L237)
- [Node receivers warn once when a listener count exceeds their local maximum](../src/inherited-emitter.test.ts#L260)
- [Namespace removal and filtered counts follow Node EventEmitter](../src/inherited-emitter.test.ts#L313)
- [Node eventNames uses property-key order for integers, strings, and symbols](../src/inherited-emitter.test.ts#L334)
- [Namespace meta-events collide with Socket.IO reserved outgoing names](../src/inherited-emitter.test.ts#L351)
- [Server delegates the newListener collision to the root Namespace](../src/inherited-emitter.test.ts#L368)
- [bulk removal of the final removeListener observer follows the host and receiver](../src/inherited-emitter.test.ts#L421)
...
- [the url query wins wholesale over the options query when both are given](../src/connect-url.test.ts#L172)
- [the options query is used only when the url carries none](../src/connect-url.test.ts#L184)
- [the url's path selects the namespace](../src/connect-url.test.ts#L192)
- [connect(url) rejects an unregistered namespace without creating membership](../src/connect-url.test.ts#L202)
- [a relative url resolves against location.origin](../src/connect-url.test.ts#L230)
- [connect(url) to an unregistered origin fires connect_error, without throwing](../src/connect-url.test.ts#L248)
- [a failed client still rejects reserved names on every emit wrapper](../src/connect-url.test.ts#L268)
- [close unregisters the server so later connect(url) reports a missing server](../src/connect-url.test.ts#L289)
- [closing a replaced server does not unregister its replacement](../src/connect-url.test.ts#L310)
- [the socket from a failed connect still chains](../src/connect-url.test.ts#L322)
- [a failed client carries the complete catch-all listener surface](../src/connect-url.test.ts#L358)
- [a failed client carries component-emitter listener introspection](../src/connect-url.test.ts#L389)
...
- [Adapter ignores deletion for membership it does not hold](../src/adapter.test.ts#L50)
- [broadcast routing ignores stale sids returned by a custom adapter](../src/adapter.test.ts#L58)
- [io.adapter registers a custom adapter that observes the routing decision](../src/adapter.test.ts#L72)
- [a custom adapter can drop a socket from the target set, and per-socket order still holds](../src/adapter.test.ts#L95)
- [registering a custom adapter preserves per-socket delivery order](../src/adapter.test.ts#L123)
- [builds an independent registered adapter for each dynamic concrete child](../src/adapter.test.ts#L147)
...
- [shared-worker protocol validates message shape, direction, and protocol version at both boundaries](../src/shared-worker.test.ts#L114)
- [shared-worker host connects through the existing server and carries acknowledgements both ways](../src/shared-worker.test.ts#L216)
- [shared-worker host keeps client events FIFO through an acknowledgement marker](../src/shared-worker.test.ts#L280)
- [shared-worker host ends the old generation and suppresses its late events and acknowledgements](../src/shared-worker.test.ts#L306)
- [shared-worker host reports malformed and unexpected messages without stopping the port](../src/shared-worker.test.ts#L387)
- [shared-worker host reports host delivery failures and survives an undeliverable report](../src/shared-worker.test.ts#L423)
- [shared-worker host reports connection rejection and releases the failed generation](../src/shared-worker.test.ts#L488)
- [shared-worker host disconnects a generation replaced before its connection callback](../src/shared-worker.test.ts#L527)
- [shared-worker host closes an active host once and preserves its shutdown reason](../src/shared-worker.test.ts#L586)
- [shared-worker host disconnects explicitly with the page-supplied reason](../src/shared-worker.test.ts#L606)
...
- [shared-worker client facade connects automatically, snapshots auth, buffers emits, and reconnects with a fresh id](../src/shared-worker-client.test.ts#L88)
- [shared-worker client facade matches the supported ordinary and incoming catch-all listener behavior](../src/shared-worker-client.test.ts#L128)
- [shared-worker client facade carries callback, promise, send, and server acknowledgements exactly once](../src/shared-worker-client.test.ts#L207)
- [shared-worker client facade drops stale generation traffic and retained acknowledgements before a later marker](../src/shared-worker-client.test.ts#L246)
- [shared-worker client facade reports invalid and non-cloneable traffic without stopping later delivery](../src/shared-worker-client.test.ts#L302)
- [shared-worker client facade finishes an immediate disconnect after the initial admission](../src/shared-worker-client.test.ts#L372)
- [shared-worker client facade stays disconnected when the initial port post fails](../src/shared-worker-client.test.ts#L389)
- [shared-worker client facade retains the rest of a buffered batch when delivery changes connection state](../src/shared-worker-client.test.ts#L409)
- [shared-worker client facade drops a server acknowledgement after local connection state ends](../src/shared-worker-client.test.ts#L458)
- [shared-worker client facade reports admission failure once and uses current auth on an explicit retry](../src/shared-worker-client.test.ts#L480)
- [shared-worker client facade disconnects once on pagehide and releases page lifecycle ownership](../src/shared-worker-client.test.ts#L503)
