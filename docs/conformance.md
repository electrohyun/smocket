...
- [shared-worker host keeps client events FIFO through an acknowledgement marker](../src/shared-worker.test.ts#L283)
- [shared-worker host ends the old generation and suppresses its late events and acknowledgements](../src/shared-worker.test.ts#L309)
...
- [shared-worker client facade connects automatically, snapshots auth, buffers emits, and explicitly replaces the active generation](../src/shared-worker-client.test.ts#L88)
...
- [shared-worker client facade carries callback, promise, send, and server acknowledgements exactly once](../src/shared-worker-client.test.ts#L214)
- [shared-worker client facade drops stale generation traffic and retained acknowledgements before a later marker](../src/shared-worker-client.test.ts#L253)
- [shared-worker client facade reports invalid and non-cloneable traffic without stopping later delivery](../src/shared-worker-client.test.ts#L309)
- [shared-worker client facade ignores host messages that do not belong to the current local state](../src/shared-worker-client.test.ts#L379)
- [shared-worker client facade reports a server-initiated disconnect while locally connected](../src/shared-worker-client.test.ts#L474)
- [shared-worker client facade finishes an immediate disconnect after the initial admission](../src/shared-worker-client.test.ts#L487)
- [shared-worker client facade stays disconnected when the initial port post fails](../src/shared-worker-client.test.ts#L504)
- [shared-worker client facade stops flushing a buffered batch when delivery changes connection state](../src/shared-worker-client.test.ts#L524)
- [shared-worker client facade drops a server acknowledgement after local connection state ends](../src/shared-worker-client.test.ts#L574)
- [shared-worker client facade reports admission failure once and uses current auth on an explicit retry](../src/shared-worker-client.test.ts#L602)
- [shared-worker client facade disconnects once on pagehide and releases page lifecycle ownership](../src/shared-worker-client.test.ts#L625)
