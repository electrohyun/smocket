...
> pre-1.0 packages, GitHub Actions, and every minor or major stay manual.
...
`pnpm.catalog.default` when the current version is at least 1.0.0.
...
- **Automerge every non-major.** Minor toolchain releases and pre-1.0 patches can carry
  compatibility decisions wider than the automated exception.
