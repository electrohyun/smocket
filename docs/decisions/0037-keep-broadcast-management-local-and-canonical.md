...
**Status:** Accepted · 2026-08-14 · #265, #322, #323
...
> `socketsLeave`, while deferring deprecated `allSockets`. Management selects canonical
> local Sockets; custom routing and delivery filters do not redefine that set.
...
the selected sockets synchronously and returned `undefined`. `allSockets` was deprecated
and ignored exclusions in both versions.

Smocket implements `fetchSockets`, `socketsJoin`, and `socketsLeave`. It defers
`allSockets` instead of adding a deprecated API whose compatibility requires preserving
an exclusion quirk. Bulk disconnect remains tracked by #324 and is not accepted by this
record.
