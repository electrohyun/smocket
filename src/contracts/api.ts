 ...
 * in `parity.ts` prove socket.io itself still satisfies each contract, so the
 ...
 * beside the projections in `parity.ts`.
 */
...
export interface ServerSocketReservedEvents {
...
export interface ClientSocketReservedEvents {
  connect: () => void;
 ...
 * to the `Ensure<>` checks in `parity.ts`. Real socket.io's adapter also delivers and needs
 ...
 * verified against, and the `Ensure<>` proofs in `parity.ts` stop compiling the
 ...
 * and the `Ensure<>` guard in `parity.ts` holds.
 */
