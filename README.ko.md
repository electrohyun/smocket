...
> **상태: 1.0.0 준비 중.** 메모리 안에서 동작하는 전달·라우팅 핵심부를
> dual-run 적합성 테스트로 실제 Socket.IO와 비교하고 있습니다. 공개 API는
> 1.0.0 전까지 바뀔 수 있습니다. [로드맵](docs/roadmap.md)과
> [버전이 보장하는 범위](docs/conformance.md#what-a-version-number-promises)를 확인해 주세요.

## 백엔드 전에 이벤트 흐름 개발하기

백엔드 이벤트 API가 준비되기 전에도 채팅 패널을 그릴 수는 있습니다. 하지만
컴포넌트를 더 렌더링하는 것만으로 여러 클라이언트를 연결하고, 같은 room에 참여시키고,
대상에 따라 서로 다른 이벤트를 받게 만들 수는 없습니다. 백엔드를 기다리는 동안에는
이런 상호작용의 전제를 프론트엔드에서 확인하기 어렵습니다.

직접 만든 socket 객체는 첫 listener와 emit에는 쓸 수 있습니다. 그러나 보통 하나의
handler map만 가지므로 room 참여 상태, namespace, broadcast 대상, acknowledgement의
생명주기를 바탕으로 수신자를 정하지 못합니다. socket 모양의 객체를 더 추가하면 전달
규칙까지 애플리케이션 fixture에서 직접 유지해야 합니다.
